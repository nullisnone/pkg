module yusur_hash_allocation (
    ...
    //temp regiters for improving timing
    reg [2047:0] mem_A_read;
    reg [16383:0] mem_B_read;

    //for record current trigger stage
    reg trigger_0 = 0;
    reg trigger_1 = 0;
    reg trigger_2 = 0;
    reg [15:0] trigger_stage = 0;
    // -------------------------------------------------------------------------
    // Main FSM & Logic
    // -------------------------------------------------------------------------
                ...
                STATE_RECEIVE: begin
                    ...
                    if (write_enable) begin
                        if (addr_20 < 20'h10000) begin
                            // Write Matrix 
                            state <= STATE_RECEIVE;
                            write_counter[15:0] <= write_counter[15:0] + 1;
                        end else if (addr_20>=20'h10000 && addr_20<20'h10200) begin 
                            state <= STATE_RECEIVE;
                            trigger_stage <= trigger_stage | (1'b1<<addr_20[8:5]);
                        end else if (trigger_stage==16'hffff) begin
                            state <= STATE_CALC;
                            trigger_counter[15:0] <= trigger_counter[15:0] + 1;
                            trigger_stage <= 0;
                        end else begin
                            state <= STATE_RECEIVE;
                            write_counter[31:16] <= write_counter[31:16] + 1;
                        end
                    end else begin
                        state <= STATE_RECEIVE;
                    end
    ...
    // Output FSM
    // -------------------------------------------------------------------------
    wire [12:0] next_out_idx = out_idx[12:0] + 1;
            ...
            case (out_state)
                OUT_STATE_IDLE: begin
                    ...
                    if (state_7d == STATE_CALC) begin // Triggers after 6-stage adder pipeline + write
                        ...
                        o_axiu_user0_data_TKEEP <= 32'hffff_ffff;
                ...
                OUT_STATE_OUTPUT: begin
                    ...
                    if (o_axiu_user0_data_TREADY) begin
                        if (out_idx == 13'h1fff) begin // 8191
                            ...
                            o_axiu_user0_data_TDATA <= {mem_C_3[next_out_idx], mem_C_2[next_out_idx], 
                                                        mem_C_1[next_out_idx], mem_C_0[next_out_idx]};
                        end
                    end
                end
            endcase
    ...
    always @(posedge ap_clk) begin
        if(state_6d==STATE_CALC && calc_loop_6d == 0) begin    
            th0_result <= {sum_result[7], sum_result[6], sum_result[5], sum_result[4], 
                            sum_result[3], sum_result[2], sum_result[1], sum_result[0]};
        end
    end        
