    ...
    for (int i = start; i < end; i++) {
        for (int j = 0; j < SIZE; j++) {
            ...
            for (int k = 0; k < SIZE; k += 8) {
                ...
                __m256i v_m1 = _mm256_load_si256((__m256i*)&data->m1[i][k]);
                __m256i v_m2 = _mm256_load_si256((__m256i*)&data->m2[j][k]);
            ...
            data->result[i][j] = _mm_cvtsi128_si32(v_sum128);
        }
    }
...
int main() {
    ...
    save_matrix_to_csv("./output-matrix-avx2-8threads.csv", result);

    printf("\nCompute Time (AVX2 with 8 Threads): %.3f ms\n", time_taken * 1000);
